...
import { BackendProject } from '@/config/backend-projects'
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'
import { Globe, Github, BookOpen } from 'lucide-react'
import { useTranslations, useLocale } from 'next-intl'
import { getProjectIcon, getUpdateTypeIcon } from '@/lib/backend-icons'
...
export function BackendProjectCard({ project, latestUpdate, onClick }: BackendProjectCardProps) {
  ...
  // 获取项目图标组件
  const ProjectIcon = getProjectIcon(project.logo)
      ...
      <CardHeader className="pb-3 p-4 md:p-6">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2 md:gap-3">
            {/* 项目图标 */}
            <div className={`flex items-center justify-center flex-shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-lg bg-gradient-to-br ${project.color || 'from-gray-500 to-gray-600'}`}>
              <ProjectIcon className="h-6 w-6 md:h-7 md:w-7 text-white transition-transform group-hover:scale-110" />
            </div>
        ...
        {latestUpdate && (() => {
          ...
          return (
            <div className="flex flex-wrap items-center gap-2 text-xs md:text-sm">
              <span className={`flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium border bg-primary/5 border-primary/20 text-primary`}>
